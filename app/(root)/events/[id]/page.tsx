    ...
    <>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 2xl:max-w-7xl">
          ...
          <div className="flex w-full flex-col gap-8 p-5 md:p-10">
            <div className="flex flex-col gap-6">
              <h2 className="h2-bold">{event.data?.title}</h2>

              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div className="flex gap-3">
                  <p className="p-bold-20 rounded-full bg-green-500/10 px-5 py-2 text-green-700">
                    {event.data?.isFree ? "FREE" : `$${event.data?.price}`}
                  </p>
            ...
            <div className="flex flex-col gap-5">
              <div className="flex gap-2 md:gap-3">
                ...
                <div className="p-medium-16 lg:p-regular-20 flex flex-wrap items-center">
                  <p>
                    {formatDateTime(event.data?.startDateTime).dateOnly} -{" "}
                    {formatDateTime(event.data?.startDateTime).timeOnly}
                  </p>
                  <p>
                    {formatDateTime(event.data?.endDateTime).dateOnly} -{" "}
                    {formatDateTime(event.data?.endDateTime).timeOnly}
                  </p>
                </div>
              </div>
      ...
      <section className="wrapper my-8 flex flex-col gap-8 md:gap-12">
        ...
        <Collection
          ...
          limit={3}
          page={searchParams.page as string}
          totalPages={relatedEvents.data?.totalPages}
        />
      </section>
    </>
  );
